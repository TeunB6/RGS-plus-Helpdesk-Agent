...
| **`sam-meta` leaks on the widget path** | `widget/rgsplus-chat.js` iframes the agent UI directly, so nothing strips the trailer and the customer sees a code block under every answer. The bridge path is unaffected. The renderer fix belongs in the chat UI — see the note in `bridge/app/agent_meta.py`. Still unfixed, but no longer un-fixable *here*: the renderer lives in `hermes-webui`, which `agent/Dockerfile` clones at build time, and that Dockerfile is now vendored — so it could be patched by injection, the way `uppr-download.js` already is |
| ~~**`openpyxl` is not installed by the base image**~~ **RESOLVED** | The base image is vendored under `agent/` now, so `agent/Dockerfile` installs `openpyxl` (with `uv` — that venv has no pip) and asserts the import at build time. This was the concrete payoff of vendoring: previously it could only be *warned* about at staging time |
